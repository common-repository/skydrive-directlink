 ...
 */
 ...
 /**
 * I have to modify this file for skydrive directlink plugin.
 * --flarefox 2011.5.29
 */
class SDDL_WP_Http {
	...
	function __construct() {
		SDDL_WP_Http::_getTransport();
		SDDL_WP_Http::_postTransport();
	}
	...
	function &_getTransport( $args = array() ) {
		...
		if ( is_null($working_transport) ) {
			if ( true === SDDL_WP_Http_ExtHttp::test($args) ) {
				$working_transport['exthttp'] = new SDDL_WP_Http_ExtHttp();
				$blocking_transport[] = &$working_transport['exthttp'];
			} else if ( true === SDDL_WP_Http_Curl::test($args) ) {
				$working_transport['curl'] = new SDDL_WP_Http_Curl();
				$blocking_transport[] = &$working_transport['curl'];
			} else if ( true === SDDL_WP_Http_Streams::test($args) ) {
				$working_transport['streams'] = new SDDL_WP_Http_Streams();
				$blocking_transport[] = &$working_transport['streams'];
			} else if ( true === SDDL_WP_Http_Fopen::test($args) ) {
				$working_transport['fopen'] = new SDDL_WP_Http_Fopen();
				$blocking_transport[] = &$working_transport['fopen'];
			} else if ( true === SDDL_WP_Http_Fsockopen::test($args) ) {
				$working_transport['fsockopen'] = new SDDL_WP_Http_Fsockopen();
	...
	function request( $url, $args = array() ) {
		...
		if ( ! is_array( $r['headers'] ) ) {
			$processedHeaders = SDDL_WP_Http::processHeaders( $r['headers'] );
		...
		if ( SDDL_WP_Http_Encoding::is_available() )
			$r['headers']['Accept-Encoding'] = SDDL_WP_Http_Encoding::accept_encoding();
		...
		if ( empty($r['body']) ) {
			...
			$transports = SDDL_WP_Http::_getTransport( $r );
		} else {
			...
			// limitations.
			$transports = SDDL_WP_Http::_postTransport( $r );
		}
	...
	function processHeaders($headers) {
		...
		foreach ( $headers as $tempheader ) {
			...
			if ( !empty( $value ) ) {
				...
				if ( 'set-cookie' == $key )
					$cookies[] = new SDDL_WP_Http_Cookie( $value );
			}
		}
	 ...
	 *
	 * If it's found, then it's assumed to contain SDDL_WP_Http_Cookie objects, which are each parsed
	 ...
	 *
	 * You block external URL requests by defining SDDL_WP_Http_BLOCK_EXTERNAL as true in your wp-config.php
	...
	function block_request($uri) {
		...
		if ( ! defined( 'SDDL_WP_Http_BLOCK_EXTERNAL' ) || ! SDDL_WP_Http_BLOCK_EXTERNAL )
			return false;
 ...
 * @since 2.7.0
 */
class SDDL_WP_Http_Fsockopen {
	/**
	 * Send a HTTP request to a URI using fsockopen().
	 *
	 * Does not support non-blocking mode.
	 *
	 * @see SDDL_WP_Http::request For default options descriptions.
	 *
	 * @since 2.7
	 * @access public
	 * @param string $url URI resource.
	...
	function request($url, $args = array()) {
		...
		$strResponse = '';
		$maxlength = $r['maxresplength'];
		$count = 0;
		while ( ! feof($handle) ) {
			$strResponse .= fread($handle, 1024);
			if ($maxlength > 0) {
				$count++;
				if ($count * 1024 >= $maxlength)
					break;
			}
		}
		...
		$arrHeaders = SDDL_WP_Http::processHeaders($process['headers']);
		...
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] )
			$process['body'] = SDDL_WP_Http::chunkTransferDecode($process['body']);

		if ( true === $r['decompress'] && true === SDDL_WP_Http_Encoding::should_decode($arrHeaders['headers']) )
			$process['body'] = SDDL_WP_Http_Encoding::decompress( $process['body'] );
...
class SDDL_WP_Http_Fopen {
	 ...
	 *
	 * @see SDDL_WP_Http::retrieve For default options descriptions.
	 *
	 * @access public
	 * @since 2.7.0
	 *
	...
	function request($url, $args = array()) {
		...
		$processedHeaders = SDDL_WP_Http::processHeaders($theHeaders);

		if ( ! empty( $strResponse ) && isset( $processedHeaders['headers']['transfer-encoding'] ) && 'chunked' == $processedHeaders['headers']['transfer-encoding'] )
			$strResponse = SDDL_WP_Http::chunkTransferDecode($strResponse);

		if ( true === $r['decompress'] && true === SDDL_WP_Http_Encoding::should_decode($processedHeaders['headers']) )
			$strResponse = SDDL_WP_Http_Encoding::decompress( $strResponse );
...
class SDDL_WP_Http_Streams {
	...
	function request($url, $args = array()) {
		...
		$strResponse = stream_get_contents($handle, $r['maxresplength']);
		$meta = stream_get_meta_data($handle);
		...
		if ( isset( $meta['wrapper_data']['headers'] ) )
			$processedHeaders = SDDL_WP_Http::processHeaders($meta['wrapper_data']['headers']);
		else
			$processedHeaders = SDDL_WP_Http::processHeaders($meta['wrapper_data']);
...
class SDDL_WP_Http_ExtHttp {
	...
	function request($url, $args = array()) {
		...
		if ($r['maxresplength'] > 0)
			$options['maxfilesize'] = $r['maxresplength'];
		...
		$headers_body = SDDL_WP_Http::processResponse($strResponse);
		$theHeaders = $headers_body['headers'];
		$theBody = $headers_body['body'];
		unset($headers_body);

		$theHeaders = SDDL_WP_Http::processHeaders($theHeaders);
		...
		if ( true === $r['decompress'] && true === SDDL_WP_Http_Encoding::should_decode($theHeaders['headers']) )
...
//		curl_setopt( $handle, CURLOPT_SSL_VERIFYPEER, $ssl_verify );
		...
		// hack：修改theHeaders的文件大小为真实大小
		// 若返回的为206，partial request，则查找content-range列
		if ($response['code'] == 206) {
			$range = $theHeaders['headers']['content-range'];
			if ( !empty($range) ) {
				$start = stripos($range, "/");
				$len = substr($range, $start+1);
				$theHeaders['headers']['content-length'] = $len;
			}
		}
		...
		if ( true === $r['decompress'] && true === SDDL_WP_Http_Encoding::should_decode($theHeaders['headers']) )
			$theBody = SDDL_WP_Http_Encoding::decompress( $theBody );
 ...
 * already a SDDL_WP_Http_Cookie() object, then they are turned into one.
 *
...
class SDDL_WP_Http_Cookie {
	 ...
	 */
	function SDDL_WP_Http_Cookie( $data ) {
...
class SDDL_WP_Http_Encoding {
	...
	function decompress( $compressed, $length = null ) {
		...
		if ( false !== ( $decompressed = SDDL_WP_Http_Encoding::compatible_gzinflate( $compressed ) ) )
			return $decompressed;
